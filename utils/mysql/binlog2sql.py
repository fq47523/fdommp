...
def generate_sql_pattern(binlog_event, row=None, flashback=False, no_pk=False):
    ...
    if flashback is True:
        if isinstance(binlog_event, WriteRowsEvent):
            ...
            print(' AND '.join(map(compare_items, row['before_values'].items())))
            template = 'UPDATE `{0}`.`{1}` SET {2} WHERE {3} LIMIT 1;'.format(
                binlog_event.schema, binlog_event.table,
                ', '.join(['`%s`=%%s' % x for x in row['after_values'].keys()]),
...
if __name__ == '__main__':
    ...
    # for ds in sqlList:
    #     print(ds)
    conn_setting = {'host': '192.168.79.134', 'port': 3307, 'user': 'root', 'passwd': 'redhat', 'charset': 'utf8'}
    binlog2sql = Binlog2sql(connection_settings=conn_setting,
                            back_interval=1.0, only_schemas='opsmanage',
                            ...
                            start_file='mysql-bin.000005',
                            start_time='2019-12-01 12:25:31',
                            stop_time='2019-12-01 14:45:31', )
    sqlList = binlog2sql.process_binlog()
    for ds in sqlList:
        print(ds)
